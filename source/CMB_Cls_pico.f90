    ...
    Transfer_SortAndIndexRedshifts  !JD added for nonlinear lensing of CMB + MPK compatibilit\y  
    use Errors !CAMB
    use settings
    use IO
    use likelihood
    use powerspec
    !! Use Pico's interface for CAMB
    use pico_camb
    implicit none

    logical :: CMB_lensing = .false.
    logical :: Imitate_lensing = .false.
    ...
    integer :: lmax_computed_cl = lmax_camb !value used for CAMB

    integer, parameter :: ScalClOrder(5) = (/1,3,2,4,5/), TensClOrder(4) = (/1,4,2,3/)
    !Mapping of CAMB CL array ordering to TT , TE, EE, BB, phi, phiT
    integer :: ncalls = 0,ncalls_pico=0
    integer :: nerrors = 0
    type(CAMBParams)  CAMBP

    real(mcp), allocatable :: highL_lensedCL_template(:,:), delta_lensedCL_template(:)
...
!changes to neutrino Neff 9/12/2013
!        P%Num_Nu_Massive = 0
!        P%Num_Nu_Massless = CMB%nnu
!9/12: was massive=3, massless=nnu-3
!10/7: Massive=nnu; Massless=0
        P%Num_Nu_Massive = 3
        P%Num_Nu_Massless = CMB%nnu-3
        if (CMB%omnuh2>0) then
           P%Nu_mass_eigenstates=0
           if (CMB%omnuh2>CMB%omnuh2_sterile) then
              neff_massive_standard = 3.0 !num_massive_neutrinos*default_nnu/3
              P%Num_Nu_Massive = 3 ! num_massive_neutrinos
...
!        print*,'cmbtocamb',P%Num_Nu_Massless,P%Num_Nu_Massive
        P%YHe = CMB%YHe
#ifdef COSMOREC
    ...
    ncalls=ncalls+1
    if (Info%Transfers%set_by_PICO) &
         ncalls_pico=ncalls_pico+1

    if (MPIRank == 0 .and. ncalls < 5) then
       print*,'PICO called ',ncalls_pico, ' times; ', &
            'CAMB called ',ncalls-ncalls_pico, ' times; ', &
            nerrors,' errors'
    endif
    if (mod(ncalls,100)==0 .and. logfile_unit/=0) then
       write (logLine,*) 'PICO called ',ncalls_pico, ' times; ', &
            'CAMB called ',ncalls-ncalls_pico, ' times; ', &
            nerrors,' errors'
       call IO_WriteLog(logfile_unit,logLine)
    end if
    if (Feedback > 1) write (*,*) 'CAMB done'
!    print*,'At end of GetNewTransferData',Info%Transfers%ClTransScal%ls%l0
    end subroutine GetNewTransferData

    subroutine GetNewPowerData(CMB, Info, Theory, error)
    use ModelParams, only : ThreadNum
    type(CMBParams) CMB
    integer error,i,imin,imax
    Type(ParamSetInfo) Info
    Type(TheoryPredictions) Theory

    if (.not. skipCAMB) then
       call SetCAMBInitPower(Info%Transfers%Params,CMB,1)

    ! pre-swap to pico
    !    call CAMB_TransfersToPowers(Info%Transfers)
!    print*,'In GetNewPowerData',Info%Transfers%ClTransScal%ls%l0
       call Pico_TransfersToPowers(Info%Transfers)
    endif
...
!          open(33,file='negativeClsDebug.dat')
!          imin=lbound(Theory%cl,1)
!          imax=ubound(Theory%cl,1)
!          do i=imin,imax
!             write(33,*)i,Theory%cl(i,1)
!          enddo
!          close(33)
!#ifdef _PICO_FORCE_
!          print*,'warning, negative C_l - zeroing entire spectrum'
!          Theory%cl(:,1)=0
!#else
!          call MpiStop('CMB_cls_pico: negative C_l (could set error here)')
!#endif
          Theory%cl(:,1)=0
          error = 1
          return
...
!        print*,'gettheory reion:',CMB%tau,CMB%zre
        call Pico_GetResults(P)
    ...
    if (.not. skipCAMB) then
       ...
       if (CMB_Lensing) then
          !assume this is always smaller than Cl_Scalar
          !+1 because ells starts at 2, not 1
          lmax_eff = size(Cl_lensed,1)+1
       else
          lmax_eff = size(Cl_scalar,1)+1 
       endif
       !pico will sometimes return extras...
       if (lmax_computed_cl .lt. lmax_eff) lmax_eff = lmax_computed_cl
       lmax_eff = lmax_eff-2 !drop last ones just in case
       Theory%cl=0
       do l = 2, lmax_eff
       ...
       if (lmax_eff/=lmax) then
          !use template for very high L theory tail, scaled not to be discontinuous                                                                 
          highL_norm = Theory%cl(lmax_eff,1)/highL_lensedCL_template(lmax_eff,1)
          Theory%cl(lmax_eff+1:lmax,1:num_ClsS) =  highL_norm*highL_lensedCL_template(lmax_eff+1:lmax,1:num_clsS)
       endif
    else
       lmax_eff = lmax_computed_cl
       Theory%tensor_ratio_02 = 0
       Theory%tensor_ratio_r10 = 0
       Theory%cl(2:lmax,1:num_ClsS) =  highL_lensedCL_template(2:lmax,1:num_clsS)
    endif

    !add lensing to unlensed
    if (CMB_lensing == .false. .and. Imitate_lensing == .true.) then 
       Theory%cl(2:lmax,1) = Theory%cl(2:lmax,1) +delta_lensedCL_template(2:lmax)
    endif
...
!    print*,'GetZreFromTau: ',GetZreFromTau, tau
...
!    use mpk
    ...
    !10/7 (was 3/.046 before)
!10/8 returned to 3/0.046
    P%Num_Nu_Massive = 3
...
500   close(tmp_file_unit)
      ...
      fname = ReadIniFilename(DefIni,'delta_lensed_cl_template',DataDir,.false.)
      allocate(delta_lensedCL_template(2:lmax))
      delta_lensedCL_template(:)=0.0
      if (fname /='') then
         call OpenTxtFile(fname,tmp_file_unit)
         do
            read(tmp_file_unit,*, end=501) L , ftmp
            if (L>lmax) exit
            nm = 2*pi/(l*(l+1))
            if (L>=2) delta_lensedCL_template(L) = nm*ftmp
         end do
501      close(tmp_file_unit)
      endif
    ...
    end subroutine ParamSetInfo_Clear
    ...
    Imitate_lensing = Ini_Read_Logical('Imitate_lensing',Imitate_lensing)
    ...
    lmax_computed_cl = Ini_Read_Int('lmax_computed_cl',lmax_camb)
    if (lmax_computed_cl /= lmax) then
     ...
     skipCAMB = (lmax_computed_cl < 600)
     if (skipCAMB .and. use_LSS) call MpiStop('trying to skip camb while using camb outputs (LSS)')
    ...
    if (Feedback > 0 .and. MPIRank==0) then
        ...
        write(*,'(" lmax              = ",1I5)') lmax
        write(*,'(" lmax_computed_cl  = ",1I5)') lmax_computed_cl
    ...
    subroutine AcceptReject(accpt, CurParams, Trial)
      use Transfer, only : Transfer_Free
      use ModelData, only : Free_ClTransfer
      ...
      if (.not. associated(CurParams%Transfers%ClTransScal%Delta_p_l_k,&
         ...
         if (accpt) then
            call Free_ClTransfer(CurParams%Transfers%ClTransScal)
            call Free_ClTransfer(CurParams%Transfers%ClTransVec)
            call Free_ClTransfer(CurParams%Transfers%ClTransTens)
         else
            call Free_ClTransfer(Trial%Transfers%ClTransScal)
            call Free_ClTransfer(Trial%Transfers%ClTransVec)
            call Free_ClTransfer(Trial%Transfers%ClTransTens)
         endif
      endif
      if (.not. associated(CurParams%Transfers%Mtrans%q_trans,Trial%Transfers%Mtrans%q_trans)) then 
         if (accpt) then
            call Transfer_Free(CurParams%Transfers%Mtrans)
         else
            call Transfer_Free(Trial%Transfers%Mtrans)
         endif
      endif
