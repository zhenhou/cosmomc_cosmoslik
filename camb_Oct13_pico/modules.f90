    ...
    subroutine Init_Limber(CTrans)
      Type(ClTransferData) :: CTrans
      if (CTrans%NumSources < 3+num_redshiftwindows) then
         print*,'Init_limber:',CTrans%NumSources,3,num_redshiftwindows
         call MPIStop('Init_limber quitting because initialized too small array')
      endif
    ...
    subroutine Free_Limber(CTrans)
      ...
      if (associated(CTrans%Limber_l_min) .and. associated(CTrans%Limber_windows)) then
